# 문서 안내

## 개요

이 디렉터리는 React 템플릿의 구조, 기술 선택, 테스트 전략, 기능 추가
기준을 설명합니다.

로컬 설정과 전체 예제 경로는 [개발 및 실행 가이드](./development-guide.md)를 참고하세요.

## 권장 읽기 순서

1. [프로젝트 아키텍처](./architecture.md)
2. [기술 스택](./tech-stack.md)
3. [테스트와 검증](./testing.md)
4. [새 기능 추가 체크리스트](./feature-addition-checklist.md)
5. [pnpm 명령](./PNPM_COMMAND.md)
6. [기여 가이드](../CONTRIBUTING.md)
7. [대시보드 예제와 집계 기준](./dashboard-examples.md)
8. [디자인 시스템](./design-system.md)
9. [사용자·주문·상품 관리 예제](./management-examples.md)
10. [Blog / Docs 예제](./blog-docs-examples.md)
11. [E-commerce 예제](./ecommerce-examples.md)
12. [랜딩 페이지 예제](./landing-examples.md)
13. [React 19 적용 전후 가이드](./react-19-modernization.md)

## 목적별 문서

- 새 route나 feature를 추가할 때:
  [아키텍처](./architecture.md),
  [기능 추가 체크리스트](./feature-addition-checklist.md)
- API와 상태 관리 경계를 확인할 때:
  [아키텍처](./architecture.md)
- 테스트 계층과 품질 게이트를 확인할 때:
  [테스트와 검증](./testing.md)
- 로컬 명령을 빠르게 찾을 때:
  [pnpm 명령](./PNPM_COMMAND.md)
